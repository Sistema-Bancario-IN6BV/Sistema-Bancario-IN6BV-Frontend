// ProfilePage.jsx — FIX: actualizar authStore después de guardar
// El backend SÍ guarda correctamente (confirmado por Network tab).
// El problema: handleSubmit hacía window.location.reload() que recarga
// la página y Zustand persist rehidrata desde localStorage con los datos
// VIEJOS (porque el store no se había actualizado antes del reload).
// Fix: usar useAuthStore.setState() para actualizar user en el store
// con los datos frescos que devuelve el backend, sin recargar la página.
...
const labelBase = {
  ...
  textTransform: 'uppercase', color: 'rgba(232,240,254,0.38)',
  marginBottom: '6px', display: 'block',
};

const sectionHeader = () => ({
  padding: '16px 22px', borderBottom: '1px solid rgba(255,255,255,0.07)',
  background: 'rgba(0,0,0,0.15)', display: 'flex', alignItems: 'center', gap: '10px',
});
...
export const ProfilePage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Extraer usuario actualizado del response del backend
      const updatedUser = extractProfile(data);
      if (updatedUser) {
        // Sincronizar Zustand store (persiste en localStorage automáticamente)
        setState((s) => ({ user: normalizeAuthUser({ ...s.user, ...updatedUser }) }));
        // Actualizar form local con datos confirmados por el backend
    ...
    <div style={{
      ...
      <div style={{ maxWidth: '1100px', margin: '0 auto' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', flexWrap: 'wrap', gap: '16px', marginBottom: '28px' }}>
          ...
          <button
            type="button"
            onClick={() => { if (editing) { setFile(null); setPreviewUrl(null); setHasChanges(false); } setEditing(!editing); }}
            style={{
              ...
              <div style={{ ...glassCard, padding: '28px 20px', textAlign: 'center' }}>
                <div style={{ position: 'relative', display: 'inline-block', marginBottom: '18px' }}>
                  <div style={{
                    ...
                    background: isAdmin ? 'conic-gradient(from 0deg,#4f8ef7,#00d4a0,#4f8ef7)' : 'conic-gradient(from 0deg,#00d4a0,#a78bfa,#00d4a0)',
                    animation: 'profileRingSpin 8s linear infinite', zIndex: 0,
                  }} />
                  <img src={avatarSrc} alt={displayName}
                    onError={e => { e.currentTarget.onerror = null; e.currentTarget.src = defaultAvatarImg; }}
                    style={{ position: 'relative', zIndex: 1, width: '100px', height: '100px', borderRadius: '50%', objectFit: 'cover', border: '3px solid rgba(7,13,26,0.9)', display: 'block' }}
                  />
                  {editing && (
                    <label htmlFor="profile-pic-input" style={{
                      ...
                      justifyContent: 'center', cursor: 'pointer', color: '#e8f0fe', flexDirection: 'column', gap: '4px',
                    }}>
                      <IconCamera />
                ...
                <div style={{
                  ...
                  borderRadius: '20px', fontSize: '0.62rem', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.12em',
              ...
              {editing && (
                <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '12px' }}>
                  <button type="button"
                    ...
                    style={{ padding: '11px 22px', borderRadius: '11px', fontSize: '0.875rem', fontWeight: 600, background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.1)', color: 'rgba(232,240,254,0.6)', cursor: 'pointer' }}
                  >Cancelar</button>
...
const FormField = ({ label, name, value, onChange, readOnly, locked, help, type = 'text', fullWidth, min }) => {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '5px', gridColumn: fullWidth ? '1 / -1' : undefined }}>
      ...
      <input name={name} type={type} value={value} onChange={onChange} readOnly={readOnly} min={min}
...
const SpinnerInline = () => (
  <span style={{ display: 'inline-block', width: '14px', height: '14px', borderRadius: '50%', border: '2px solid rgba(255,255,255,0.3)', borderTopColor: '#fff', animation: 'profileRingSpin 0.7s linear infinite' }} />
);
